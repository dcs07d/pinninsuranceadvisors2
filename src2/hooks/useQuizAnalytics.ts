...
export function useQuizAnalytics() {
  ...
  const trackQuizAnswer = useCallback((questionId: number, selectedAnswer: string, questionText: string) => {
  ...
  const trackQuizComplete = useCallback((result: QuizResult) => {
  ...
  const trackConsultationRequest = useCallback((quizResult: QuizResult) => {
